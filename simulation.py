...
for _ in range(N): #몬테카를로 시뮬레이션 시행
    ...
    random_length = uniform(0,1) #0과 1사이의 랜덤 변수(x)에 대한 난수 생성 
    if(1/2 * np.sin(random_angle)>=random_length):D+=1 #1/2 * l * sin(theta) >= x이면 선과 걸침    
simulation_result = D/N #바늘이 걸치는 확률 계산 
mathematical_result = 1/np.pi #수학적 결과값
